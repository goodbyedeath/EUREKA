{{--
    ...
    Every action that used to open a modal now replaces the list with a page: create, edit,
    questions, QR and delete. Five overlays stacked on one screen were unusable on a phone,
    and the edit form in particular is far too tall for a dialog.

    The whole component is one branch chain inside a single root element. Livewire needs
    exactly one root, and the earlier version lost it — the edit state rendered two
    siblings, which is why that screen came apart on every update.
--}}
<div>

@if ($showEditModal && $editQuestionnaire)

    @include('admin.partials.back-link', ['action' => 'closeEditModal', 'label' => 'Back to questionnaires'])
    ...
    <div class="max-w-3xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5 sm:p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
            Edit Questionnaire
        </h2>
                    <form wire:submit.prevent="updateQuestionnaire" class="space-y-6">
                        ...
                        <div class="flex items-center mt-3">
                            <input type="checkbox"
                                   id="editCountsTowardFinish"
                                   wire:model.defer="editCountsTowardFinish"
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                            <label for="editCountsTowardFinish" class="ml-2 block text-sm text-gray-900 dark:text-gray-100">
                                Counts toward finishing
                            </label>
                        </div>
                        <p class="ml-6 text-xs text-gray-500 dark:text-gray-400">
                            Leave on for a normal post. Turn it off for a bonus post: it still scores,
                            but the team's race clock will not wait for it.
                        </p>
...
@elseif ($showCreateModal)

    @include('admin.partials.back-link', ['action' => 'closeCreateModal', 'label' => 'Back to questionnaires'])
...
@elseif ($showQuestionsModal && $selectedQuestionnaire)

    @include('admin.partials.back-link', ['action' => 'closeQuestionsModal', 'label' => 'Back to questionnaires'])

    {{-- Wider than the other pages: the question list carries its own table and editor. --}}
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5 sm:p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
            Manage Questions &mdash; {{ $selectedQuestionnaire->title }}
        </h2>
        <livewire:admin.question-manager :questionnaire="$selectedQuestionnaire"
                                         :key="'question-manager-'.$selectedQuestionnaire->id" />
    </div>

@elseif ($showQrModal && $qrQuestionnaire)

    @include('admin.partials.back-link', ['action' => 'closeQrModal', 'label' => 'Back to questionnaires'])

    <div class="max-w-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5 sm:p-6">
        <div class="mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $qrQuestionnaire->title }}</p>
        </div>
        <livewire:admin.q-r-code-manager :questionnaire="$qrQuestionnaire"
                                         :key="'qr-manager-'.$qrQuestionnaire->id" />
    </div>

@elseif ($showDeleteModal && $deleteQuestionnaire)

    @include('admin.partials.back-link', ['action' => 'closeDeleteModal', 'label' => 'Back to questionnaires'])
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="truncate {{ $questionnaire->max_attempts <= 1 ? 'text-red-600 font-medium' : ($questionnaire->max_attempts <= 3 ? 'text-orange-600 font-medium' : '') }}">
                                        ...
                                        @if($questionnaire->max_attempts && $questionnaire->users_at_max_attempts > 0)
                                            <span class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800" title="{{ $questionnaire->users_at_max_attempts }} teams have reached max attempts">
